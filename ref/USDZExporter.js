...
function lz4() {
  var MAX_BLOCK_INPUT_SIZE = 0x7E000000
  ...
  this.encode = function(src) {
    ...
    if (inputSize > 127 * MAX_BLOCK_INPUT_SIZE)
      ...
      dst[0] = 0
      var dst2 = this.compressDefault(src)
  ...
  this.compressDefault = function(src) {
    ...
    //var dst = new Uint8Array(this.worstCaseBlockLength(srcLen));
    var dst = new ArrayBuffer(this.worstCaseBlockLength(srcLen));
    var posTable = new PositionTable();
    var srcPtr = 0;
    var literalHead = 0;
    var dstPtr = 0;
    var MAX_INDEX = srcLen - MFLIMIT;
    while (srcPtr < MAX_INDEX) {
      var curValue = util.readU32(src, srcPtr)

      var matchPos = this.findMatch(posTable, curValue, src, srcPtr)
      if (matchPos != null) {
        var length = this.countMatch(src, matchPos, srcPtr, MAX_INDEX);
        if (length < MIN_MATCH)
          break
      ...
      //console.error("errr in compress");//var curValue = readLeUint32(src, srcPtr)
    }
  ...
  this.copySequence = function(dst, dstHead, literal, match) {
    ...
    var dstView = new DataView(dst);

    var litLen = literal.length;
    var dstPtr = dstHead;

    var token = dstView.getUint8(dstPtr, false); //littleEndian=false
    var tokenPtr = dstPtr;

    dstPtr += 1
    if (litLen >= 15) {
      token = (15 << 4)
      var remLen = litLen - 15
      while (remLen >= 255) {
        //dst[dstPtr] = 255
...
function usdInt32Compress(values) {
  ...
  var commonValue = mode(values)
  //var data=[];           //2
  var data = new ArrayBuffer(2 * values.length + 1);
  ...
  for (var v = 0; v < values.length; v++) {
    ...
    if (value != commonValue) {
      var bitLength = value === 0 ? 0 : Math.abs(value).toString(2).length;

      if (bitLength < 8) {
        var alterable = dataView.getInt8(Math.floor(i / 4));
        alterable |= (1 << ((i % 4) * 2));
...
function encodeInts(ints, size, byteOrder = true /*'little'*/ , signed = false) {
  if (!signed && size == 8) {
    var arr = new ArrayBuffer(ints.length * 8); // an Int32 takes 4 bytes

    var view = new DataView(arr);
    for (var i = 0; i < ints.length; i++) {
      //view.setBigUint64(i*8, ints[i], byteOrder);//noSafari Support
      var highLow = split64Bit(ints[i]);
...
function USDZExporter() {
  this.exportScene = function(threeScene, options) {
    ...
    var usdData = usdScene.exportUsd();
    var crate = new Crate(usdScene.textureFilePaths);
...
function Scene() {
  ...
  this.exportSharedMaterials = function(data) {
    if (Object.entries(this.materials).length > 0) {
      var looks = data.createChild('Looks', ClassType.Scope);
  ...
  this.addThreeObject = function(object, type) {
    var obj = new USDObject(object, this);
  ...
  this.addThreeCollection = function(collection) {
    var name = collection.name.replace(/[^a-zA-Z]+/g, '');
    var obj = new USDObject(collection, this);
    ...
    if (this.collections[obj.name] == undefined) {
      ...
      for (var i = 0; i < children.length; i++) {
        ...
        var childObj = new USDObject(children[i], this)
...
function USDMesh(object, scene) {
  this.createCopies = function() {
    ...
    if (this.objectCopy.geometry.isGeometry) {
      var name = this.objectCopy.geometry.name;
  ...
  this.exportToObject = function(usdObj, classType = ClassType.Mesh) {
    var mesh = this.objectCopy.geometry;
    var name = this.objectCopy.geometry.name;
    var usdMesh = usdObj.createChild(name, classType)
    ...
    var points = exportThreeMeshVertices(mesh);
    var indices = exportThreeMeshIndices(mesh);
    ...
    var normals = exportThreeMeshNormals(mesh)
    var normalIndices = exportThreeMeshNormalIndices(mesh)
    usdMesh.setItem("primvars:normals", normals);
    usdMesh.getItem("primvars:normals").valueTypeStr = "normal3f";
    usdMesh.getItem("primvars:normals").setItem("interpolation", "faceVarying");

    //DEBUG NO INDICES MAYBE NO PROBLEM? :D 'CAUSE FALLBACK TO POLY INDICES?
    usdMesh.setItem("primvars:normals:indices", indices);
  ...
  this.exportMeshUvs = function(usdMesh) {
    ...
    var name = "UVMap"; //layer.name.replace('.', '_')

    usdMesh.setItem('primvars:' + name, uvs);
    usdMesh.getItem('primvars:' + name).valueTypeStr = 'texCoord2f';
    //usdMesh.getItem('primvars:'+name).setItem("valueTypeStr", 'texCoord2f');
    //usdMesh.getItem('primvars:'+name).interpolation='faceVarying';
    usdMesh.getItem('primvars:' + name).setItem("interpolation", 'faceVarying')

    usdMesh.setItem('primvars:' + name + ':indices', indices);
...
function ShaderInput(type, name, defaultVal) { //default as third arg???
  ...
  this.exportShader = function(material, usdMaterial) {
    ...
    if (this.image != undefined && this.uvMap != undefined) {
      ...
      var usdShader = usdMaterial.createChild(this.name + "Map", ClassType.Shader)
      ...
      var primUsdShader = usdMaterial.getChild("primvar_" + this.uvMap);
...
function Material(material) {
  ...
  this.createInputs = function() {
    ...
    var defDiffuseColor = this.material.color;
    var defRoughness = this.material.roughness
    var diffuse = this.material.color;
    var specular = 1 - this.material.metalness;
    var emissive = this.material.emissive //Vec3
    ...
    var metallic = this.material.metalness
    var roughness = this.material.roughness
    var opacity = this.material.opacity
    var opacityThreshold = this.material.alphaTest
    opacityThreshold = 0;
    var ior = this.material.ior
    var useSpecular = 0 //0 if metallic > 0.0 else 1
    if (ior == undefined)
      ior = 0;
  ...
  this.exportPbrShader = function(usdMaterial) {
    var usdShader = usdMaterial.createChild('pbr', ClassType.Shader);
  ...
  this.exportPrimvar = function(usdMaterial) {

    var uvMaps = this.getUVMaps();
    for (var i = 0; i < uvMaps.length; i++) {
      var map = uvMaps[i];
      usdMaterial.setItem('inputs:frame:stPrimvar_' + map, map);
      var usdShader = usdMaterial.createChild('primvar_' + map, ClassType.Shader)
      usdShader.setItem('info:id', 'UsdPrimvarReader_float2');

      usdShader.getItem('info:id').addQualifier('uniform')
      usdShader.setItem('inputs:fallback', new THREE.Vector2(0., 0.));
      usdShader.setItem("inputs:varname", usdMaterial.getItem('inputs:frame:stPrimvar_' + map));
  ...
  this.exportUsd = function(parent) {
    ...
    this.exportPrimvar(this.usdMaterial);

    this.exportInputs(this.usdMaterial);

    var pbrShader = this.exportPbrShader(this.usdMaterial);
...
function USDObject(object, scene, type = 'EMPTY') {
  ...
  this.bakeToFile = function(type, file) {
    var texUrl;
    var image //New Parsing technology :D
    switch (type) {
      case "diff":
    ...
    /*
    for(var i=0;i<this.scene.textureFilePaths.length;i++){
  ...
  this.bakeDiffuseTexture = function() {
    if (this.materials[0] == undefined)
      return;
    var bake = false;
    var asset = this.object.material.map.uuid + "Diffuse.png"
  ...
  this.bakeNormalTexture = function() {
    ...
    var bake = false;
    var asset = this.object.material.normalMap.uuid + "Normal.png"
    asset = asset.replace(/-/g, "");
  ...
  this.bakeMetalnessTexture = function() {
    if (this.materials[0] == undefined)
      return;
    var bake = false;
    var asset = this.object.material.metalnessMap.uuid + "Metallic.png"
    asset = asset.replace(/-/g, "");
  ...
  this.bakeRoughnessTexture = function() {
    if (this.materials[0] == undefined)
      return;
    var bake = false;
    var asset = this.object.material.roughnessMap.uuid + "Roughness.png"
    asset = asset.replace(/-/g, "");
  ...
  this.bakeAoTexture = function() {
    if (this.materials[0] == undefined)
      return;
    var bake = false;
    var asset = this.object.material.aoMap.uuid + "AmbientOcclusion.png"
    asset = asset.replace(/-/g, "");
  ...
  this.bakeAlphaTexture = function() {
    if (this.materials[0] == undefined && this.materials[0].material != undefined)
      return;
    var bake = false;
    var asset = this.object.material.map.uuid + "Alpha.png"
    //var asset=this.object.material.map.uuid+"Diffuse.png"//Diffuse not Alpha, therefore no additional adding
    asset = asset.replace(/-/g, "");
  ...
  this.bakeEmissiveTexture = function() {
    if (this.materials[0] == undefined)
      return;
    var bake = false;
    var asset = this.object.material.emissiveMap.uuid + "Emissive.png"
    asset = asset.replace(/-/g, "");
  ...
  this.exportMesh = function(usdObj) {
    if (this.mesh != undefined) {
      if (this.mesh.usdMesh != undefined) {
        var usdMesh = usdObj.createChild(this.mesh.name, ClassType.Mesh);
        usdMesh.setItem("inherits", this.mesh.usdMesh);
        usdMesh.setItem("instanceable", true);
      } else {
        var usdMesh = this.mesh.exportToObject(usdObj);
...
function UsdPrim(name, type) {
  ...
  this.countItems = function() {
    var count = this.attributes.length + this.children.length;
    for (var i = 0; i < this.children.length; i++)
...
function UsdAttribute(name = '', value = null, type = ValueType.Invalid) {
  ...
  this.valueTypeToString = function() {
    if (this.valueTypeStr != undefined) {
      ...
      var ret = this.valueTypeStr
      if (this.isArray())
        ret += "[]"
      return ret;
    }
    var ret = ValueType.getName(this.valueType);
    if (this.isArray())
      ret += "[]"
    return ret;
...
function Crate(file) {
  ...
  this.writeInt = function(num, sizeInBytes, byteOrder = true /*true=littleE*/ , signed = false) {
    ...
    var arr = new ArrayBuffer(sizeInBytes); // an Int32 takes 4 bytes
    var view = new DataView(arr);
    if (sizeInBytes * 8 == 64) {
      if (!signed) {
  ...
  this.writeInt32Compressed = function(data) {
    var compi = new lz4();
    var buffer = compi.encode(usdInt32Compress(data)); //DEBUG SPEED
  ...
  this.writeUsd = function(usdData) {
    ...
    if (usdData.children.length > 0) {
      var tokens = [];
      for (var i = 0; i < usdData.children.length; i++)
    ...
    var nameToken = this.getTokenIndex('');
    var pathJump = usdData.getPathJump();
  ...
  this.writeTokensSection = function() {
    ...
    var buf = new ArrayBuffer(bufferLength); //+this.tokens.length // 2 bytes for each char
    var bufView = new Uint8Array(buf);
    ...
    var compi = new lz4();
    var compbuffer = compi.encode(buffer);
  ...
  this.writeFieldsSection = function() {
    ...
    var compi = new lz4();
    var buffer = compi.encode(encodeInts(this.reps, 8))
    this.writeInt(buffer.byteLength, 8)
    this.appendBuffer(buffer);
    var size = this.lseek - start;
  ...
  this.writeUsdPrim = function(usdPrim) {
    ...
    if (usdPrim.attributes.length > 0) {
      var tokens = [];
      for (var i = 0; i < usdPrim.attributes.length; i++)
    ...
    if (usdPrim.children.length > 0) {
      var tokens = [];
      for (var i = 0; i < usdPrim.children.length; i++)
    ...
    var nameToken = this.getTokenIndex(usdPrim.name)
    var pathJump = usdPrim.getPathJump()
    // Add Prim Path
  ...
  this.writeUsdAttribute = function(usdAtt) {
    var fset = [];
    var type = usdAtt.valueTypeToString();
    fset.push(this.addField('typeName', type))
    for (var i = 0; i < usdAtt.qualifiers.length; i++) {
      if (usdAtt.qualifiers[i] == "uniform")
        fset.push(this.addField('variability', true, ValueType.Variability))
    ...
    var nameToken = this.getTokenIndex(usdAtt.name)
    var pathJump = usdAtt.getPathJump()
  ...
  this.writeUsdConnection = function(usdAtt) {
    var fset = []
    var pathIndex = usdAtt.value.pathIndex
    fset.push(this.addField('typeName', usdAtt.valueTypeToString())) //LASTDEBUG
    /*
    for(var i=0;i<usdAtt.value.qualifiers.length;i++){
  ...
  this.writeUsdRelationship = function(usdAtt) {
    var fset = []
    var pathIndex = usdAtt.value.pathIndex
    fset.push(this.addField('variability', true, ValueType.Variability)) //LASTDEBUG
  ...
  this.addField = function(field, value, vType = ValueType.UnregisteredValue) {
    ...
    if (field == "variability" || (vType == ValueType.Variability))
      return this.addFieldVariability(field, value) ///hotfix: field should be = ValueType.Variability
    ...
    var vTypeName = Object.keys(ValueType).find(key => ValueType[key] === vType)
  ...
  this.addFieldAsset = function(field, data) {
    ...
    var token = this.getTokenIndex(data.replace('@', ''))
  ...
  this.addFieldVector = function(field, data, vType) {
    var field = this.getTokenIndex(field)
    var vTypeName = Object.keys(ValueType).find(key => ValueType[key] === vType)
    var packStr = '<' + vTypeName.substring(3, 5);
    if (Array.isArray(data)) {
  ...
  this.getStringIndex = function(str) {
    var tokenIndex = this.getTokenIndex(str)
  ...
  this.addFieldMatrix = function(field, data, vType) {
    ...
    if (1 || ref < 0) {
      ...
      this.addWritenData(data, vType, ref);
      var vTypeName = Object.keys(ValueType).find(key => ValueType[key] === vType)
      var packStr = '<' + vTypeName.substring(6, 8);
      ...
      if (Array.isArray(data)) {
        ...
        var s = struct(packStr)
        for (var i = 0; i < Math.sqrt(data.elements.length); i++) {
          var row = data.elements.slice(i * Math.sqrt(data.elements.length), i * Math.sqrt(data.elements.length) + Math.sqrt(data.elements.length));
  ...
  this.addFieldVariability = function(field, data) {
    ...
    return this.addFieldItem(field, ValueType.Variability, false, true, false, data)
  }
  ...
  this.addWritenData = function(data, vType, ref) {
    //key = [data, vType]
    var dataHash = JSON.stringify(data) + "" + vType
    var key = dataHash;
  ...
  */
  ...
  // var ARRAY_BIT = BigInt(1) << BigInt(63)
  // var INLINE_BIT = BigInt(1) << BigInt(62)
  // var COMPRESSED_BIT = BigInt(1) << BigInt(61)
  // var PAYLOAD_MASK = (BigInt(1) << BigInt(48)) - BigInt(1)
  ...
  this.generateZip = async function() {
    ...
    for (var i = 0; i < this.textureFilePaths.length; i++) {
      var image = this.textureFilePaths[i][0]
      var name = this.textureFilePaths[i][1]
      var blob;
  ...
  this.appendBuffer = function(newBuff) {
    while (this.buffer.byteLength < (this.lseek + newBuff.byteLength)) {
      var tmp = new Uint8Array(this.buffer.byteLength * 2);
  ...
  this.writeTableOfContents = function() {
    ...
    for (var i = 0; i < this.toc.length; i++) {
      ...
      var spaces = 16 - this.toc[i][0].length;
...
function exportThreeMeshVertexCounts(mesh) {
  var vertexCounts = [];
  for (var i = 0; i < (mesh.index.array.length / 3); i++) {
    vertexCounts.push(3);
  }
  return vertexCounts;
}
...
function exportThreeMeshVertices(mesh) {
  var vertices = [];
  for (var i = 0; i < mesh.attributes.position.array.length; i += 3) {
...
function exportThreeMeshIndices(mesh) {
  ...
  for (var i = 0; i < mesh.index.array.length; i++) {
...
function exportThreeMeshNormals(mesh) {
  var normals = [];
  for (var i = 0; i < mesh.attributes.normal.array.length; i += 3) {
...
function exportThreeMeshUVs(mesh) {
  ...
  var uvs = [];
  var layer = 0;
  for (var i = 0; i < mesh.attributes.uv.array.length; i += 2) {
